# Autonomous Execution Protocol — reference

Lookup material for `rules/autonomous-execution.md`. The binding trigger —
these rules apply only when a mission brief is active — lives in that rule
file and is always resident. This is the full protocol: brief structure,
startup/compaction sequence, batch execution, quality gates, decision rules,
commit discipline, progress tracking, and session end.
...
This 2-fix cap bounds attempts to fix *one specific failing gate check*.
It is a distinct counter from the consecutive-fix stop rule below, which
bounds repeated edits to *one code location/approach* across the whole
task — the two rules govern different objects and are tracked separately.
...
- Mission-brief branches use merge commits when merging to main — see
  `rules/pr-workflow.md`'s Merge strategy section for the rule (single
  source of truth; squash destroys per-task commit IDs referenced
...
3. Append a one-line stop summary to the final commit's body — `plans/`
   is gitignored, so the commit is the only durable record of why a
   mission stopped or completed.
4. The notify-on-stop hook will chime
