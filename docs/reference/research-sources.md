...
Lookup material for `rules/research-sources.md`. The binding rule — start
at Tier 1, descend only when the higher tier has no answer, plus the
confidence-declaration table — lives in that rule file and is always
resident (`CLAUDE.md` cites the table directly). This is the full tier
...
practices. Cross-reference when the claim is non-obvious (concrete test:
the claim would surprise a competent engineer unfamiliar with this
specific tradeoff).
...
When a non-obvious design decision is based on external research (concrete
test: a future maintainer reading only the code would ask "why this
approach?"):
...
understand why a non-obvious approach was chosen (concrete test: same —
if a maintainer reading only the code would ask "why," cite it).
