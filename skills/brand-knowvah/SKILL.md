...
7. **`AuthUser` fields**: if `/auth-setup` was run, extend its `AuthUser`
   interface with `consent_required?: boolean` (if compliance-setup's
   consent gate is used) and `credits_available?: number` (if this app has
   a credits model) instead of casting `user` in `App.tsx`/`Sidebar.tsx` —
   the templates below already assume this.
...
Applying the brand to a VitePress documentation site (not a React
app) is a separate, independent flow. Read
`references/vitepress-docs-site.md` for Steps D0-D6 and follow it in
full instead of the React-app steps above.
...
Files written:
  ...
  ui/public/knowvah_logo.svg             — from this skill's ui/knowvah_logo.svg template
...
After the final summary is printed, delete `.brand-knowvah-progress.md`. It is a
