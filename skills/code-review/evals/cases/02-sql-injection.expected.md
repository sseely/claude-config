...
- **Critical — line 15.** SQL injection. `email` is interpolated straight
  ...
  row. Note that `_stored_token` on line 35 uses a parameterized query
...
- **Warning — line 26.** `stored == token` is a non-constant-time comparison
...
- **Suggestion — line 37.** `_stored_token` returns `""` when no session
